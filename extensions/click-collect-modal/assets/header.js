...
async function getLocationsDropdown(selectedLocation = "") {
  console.log('test');
  try {
    const dropdownDiv = document.querySelector('.cls-pickuplocations-div'); document.querySelector('.pickup-locations-dropdown-header').classList.remove('cnc-locationsspace'); dropdownDiv.classList.add('loader');
    const pickuplcurl = `https://star-legend-fabrics-swap.trycloudflare.com/api/pickupLocation?shop=flirt-adult-store.myshopify.com`;
    ...
    if (locations && selectedLocation != null) {
      for (const location of locations) {
        if (location?.address?.zip && location?.localPickupSettingsV2 != null) { destinationsArr.push(`${location.address.address1} ${location.address.city} ${location.address.zip} ${location.address.province} ${location.address.country}`); }
      }
    ...
    if (destinationsArr.length > 0 && selectedLocation != null) {
      ...
      const mapUrl = `https://star-legend-fabrics-swap.trycloudflare.com/api/distance?customerlocation=${customerLocation}&shop=flirt-adult-store.myshopify.com`;
    ...
    } document.querySelectorAll('.pickup-locations-dropdown-header .loader').forEach(function (element) { element.classList.remove('loader'); }); document.querySelector('.pickup-locations-dropdown-header').parentElement.classList.add('cnc-cls-headericons');
...
function updateDropdownWithSelectedValue(selectedId) {
  ...
  const optionMenu = document.querySelector(".cnc-dropdwndiv .select-menu"); selectBtn = optionMenu.querySelector(".cnc-dropdwndiv .select-btn"); options = document.querySelectorAll(".cnc-dropdwndiv  .option"); sBtn_text = document.querySelector(".cnc-dropdwndiv  .sBtn-text");
  optionMenu.classList.toggle("active");
  options.forEach(optionItem2 => {
    optionItem2.addEventListener('click', (event) => {
      sBtn_text.innerHTML = event.target.innerHTML; sBtn_text.id = event.target.dataset.value;
