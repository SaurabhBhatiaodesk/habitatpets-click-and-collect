...
async function getLocations(selectedLocation = "") {
  try {
    const pickuplcurl = `https://mediterranean-adam-apt-prescription.trycloudflare.comapi/pickupLocation?shop=flirt-adult-store.myshopify.com`;
    ...
    if (destinationsArr.length > 0) {
      ...
      const mapUrl = `https://mediterranean-adam-apt-prescription.trycloudflare.comapi/distance?customerlocation=${customerLocation}&shop=flirt-adult-store.myshopify.com`;
          ...
          if (location.address.zip && location?.localPickupSettingsV2 != null) {
            ...
            for (let index = 0; index < destinationsArr.length; index++) {
              ...
              if (destinationAddress.includes(zipcode)) {
                ...
                } else if (distanceElement?.status == "OK" && distanceElement?.distance?.value > 1) { count = count + 1; }
              }
            }
          }
...
async function getInventoryLocations(callback) {
  ...
  try {
    let response = await fetch(`https://mediterranean-adam-apt-prescription.trycloudflare.comapi/cart?product_id=${productId}&shop=flirt-adult-store.myshopify.com`);
...
async function handleInventoryLocationsResponse(data) {
  ...
  for (let i = 0; i < data.product.variants.nodes.length; i++) {
    let variant = data.product.variants.nodes[i]; let variantIdParts = variant.id.split("/"); let variantId = variantIdParts[variantIdParts.length - 1];
    if (variantId === productVariantId) {
      let isInStock = false; if (variant.inventoryItem && variant.inventoryItem.inventoryLevels) {
...
async function refreshInventoryLocations() {
  try {
    getInventoryLocations((error, data) => {
      if (data) {
...
document.addEventListener("change", event => {
  if (event.target.matches(".popup-modal .address-popup input.locations")) {
    let locationName = event.target.nextElementSibling.textContent6; setCookie("storelocationName", locationName); setCookie("storelocation", event.target.id);
