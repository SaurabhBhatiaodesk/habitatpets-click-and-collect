...
async function fetchAccessToken() {
    try {
        const response = await fetch(`https://ab-remind-latinas-australia.trycloudflare.com/api/get?shop=${location.hostname}`, {
...
async function getLocations(accessToken, selectedLocation = "") {
    try {
        ...
        if (destinationsArr.length > 0) {
            ...
            const mapUrl = `https://ab-remind-latinas-australia.trycloudflare.com/api/distance?customerlocation=${customerLocation}&destinations=${destinationsArr.join("|")}&shop=${location.hostname}`;
